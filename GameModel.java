...
public class GameModel implements Cloneable {
   ...
   /**
    ...
    public void setModel(int[][] clonedModel){
    	for(int i=0;i<sizeOfGame;i++){
    		for(int j=0;j<sizeOfGame;j++){
        		model[i][j]=clonedModel[i][j];
        	}
    	}
    }
    ...
    public int[][] cloneModel() throws CloneNotSupportedException{
     int[][] cloneModel=(int[][])model.clone();
     for(int x=0;x<sizeOfGame;x++)
     {
     cloneModel[x]=((int[])cloneModel[x].clone());
     }
     return cloneModel; 
    }
